...
const uploadImage = async (imageUri, userId) => {
  try {
    console.log('Starting upload for:', imageUri, 'Platform:', Platform.OS);
    
    let arrayBuffer;
    
    // Web: använd fetch + blob
    if (Platform.OS === 'web') {
...
export const getCategoryStats = async () => {
  try {
    const all = await getObservations();
    const counts = { Växt: 0, Svamp: 0, Djur: 0, Insekt: 0, Spår: 0, Lämning: 0, Okänd: 0 };
    const animalSubs = { Däggdjur: 0, Fågel: 0, Fisk: 0, Reptil: 0, Groddjur: 0, 'Övriga smådjur': 0, 'Vet inte': 0 };
    const plantSubs = { Blomväxt: 0, 'Träd och buskar': 0, 'Gräs och örter': 0, 'Mossor och lavar': 0, 'Vet inte': 0 };
    const insectSubs = { Skalbaggar: 0, Steklar: 0, Tvåvingar: 0, Fjärilar: 0, Halvvingar: 0, Hopprätvingar: 0, Trollsländor: 0, Tvestjärtar: 0, 'Vet inte': 0 };
    const fungiSubs = { Skivlingar: 0, Soppar: 0, Tickor: 0, Taggsvampar: 0, Buksvampar: 0, Murklor: 0, 'Vet inte': 0 };
    ...
    all.forEach(obs => {
      ...
      if (obs.subcategory) {
        ...
        if (cat === 'Insekt' && insectSubs[sub] !== undefined) insectSubs[sub]++;
    ...
    return {
      counts: { Växt: 0, Svamp: 0, Djur: 0, Insekt: 0, Spår: 0, Lämning: 0, Okänd: 0 },
      animalSubs: { Däggdjur: 0, Fågel: 0, Fisk: 0, Reptil: 0, Groddjur: 0, 'Övriga smådjur': 0, 'Vet inte': 0 },
      plantSubs: { Blomväxt: 0, 'Träd och buskar': 0, 'Gräs och örter': 0, 'Mossor och lavar': 0, 'Vet inte': 0 },
      insectSubs: { Skalbaggar: 0, Steklar: 0, Tvåvingar: 0, Fjärilar: 0, Halvvingar: 0, Hopprätvingar: 0, Trollsländor: 0, Tvestjärtar: 0, 'Vet inte': 0 },
      fungiSubs: { Skivlingar: 0, Soppar: 0, Tickor: 0, Taggsvampar: 0, Buksvampar: 0, Murklor: 0, 'Vet inte': 0 },
    };
  }
};