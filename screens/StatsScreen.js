...
import { getObservations, getSeasonStats, getSpeciesStats, getCategoryStats } from '../utils/storage';
...
const StatsScreen = ({ navigation }) => {
  ...
  const visibleSpecies = showAllSpecies ? speciesStats : speciesStats.slice(0, 10);
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.heroCard}>
        ...
        <Text style={styles.progressText}>{progress}% av 365-dagars utmaningen</Text>
      </View>
              ...
              return (
                <View key={key} style={styles.animalRow}>
                  ...
                  <View style={styles.animalBarBg}>
                    <View style={[styles.animalBarFill, { width: `${pct}%` }]} />
                  </View>
             ...
             .map(({ key, emoji }) => {
              ...
              return (
                <View key={key} style={styles.animalRow}>
                  ...
                  <Text style={styles.animalName}>{key}</Text>
                  <View style={styles.animalBarBg}>
                    <View style={[styles.animalBarFill, { width: `${pct}%`, backgroundColor: '#5cb85c' }]} />
                  </View>
                  <Text style={[styles.animalCount, { color: '#5cb85c' }]}>{count}</Text>
                </View>
              );
            })}
          </View>
             ...
             .map(({ key, emoji }) => {
              ...
              return (
                <View key={key} style={styles.animalRow}>
                  ...
                  <Text style={styles.animalName}>{key}</Text>
                  <View style={styles.animalBarBg}>
                    <View style={[styles.animalBarFill, { width: `${pct}%`, backgroundColor: '#9b59b6' }]} />
                  </View>
                  <Text style={[styles.animalCount, { color: '#9b59b6' }]}>{count}</Text>
                </View>
              );
            })}
          </View>
             ...
             .map(({ key, emoji }) => {
              ...
              return (
                <View key={key} style={styles.animalRow}>
                  ...
                  <Text style={styles.animalName}>{key}</Text>
                  <View style={styles.animalBarBg}>
                    <View style={[styles.animalBarFill, { width: `${pct}%`, backgroundColor: '#8e44ad' }]} />
                  </View>
                  <Text style={[styles.animalCount, { color: '#8e44ad' }]}>{count}</Text>
                </View>
              );
            })}
          </View>
      ...
      {speciesStats.length === 0 ? (
        ...
        <View style={styles.speciesCard}>
          <Text style={styles.speciesSubtitle}>{speciesStats.length} unika arter</Text>
...
const styles = StyleSheet.create({
  ...
  // Hero
  heroCard: {
    backgroundColor: BeeColors.white, borderRadius: 18, padding: 24,
    alignItems: 'center', marginBottom: 24,
    elevation: 4, shadowColor: '#000', shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.1, shadowRadius: 8,
  },
  heroEmoji: { fontSize: 48, marginBottom: 8 },
  heroNumber: { fontSize: 56, fontWeight: 'bold', color: BeeColors.honey, lineHeight: 64 },
  heroLabel: { fontSize: 16, color: BeeColors.honeyDark, marginBottom: 16 },
  progressBarBg: {
    width: '100%', height: 12, backgroundColor: BeeColors.honeycomb,
    borderRadius: 6, overflow: 'hidden', marginBottom: 8,
  },
  progressBarFill: { height: '100%', backgroundColor: BeeColors.honey, borderRadius: 6 },
  progressText: { fontSize: 13, color: BeeColors.honeyDark },

  sectionTitle: {
    fontSize: 18, fontWeight: 'bold', color: BeeColors.honeyDark,
  ...
  seasonLabel: { fontSize: 11, color: BeeColors.white, fontWeight: '600', marginTop: 2 },

  // Kategorier
  categoryGrid: { marginBottom: 24 },
  categoryCard: {
    backgroundColor: BeeColors.white, borderRadius: 12, padding: 14,
  ...
  categoryName: { flex: 1, fontSize: 15, fontWeight: '600', color: BeeColors.honeyDark },
  categoryCount: { fontSize: 20, fontWeight: 'bold' },
  categoryBarBg: {
    height: 8, backgroundColor: BeeColors.honeycomb, borderRadius: 4,
  ...
  // Artlista
  speciesCard: {
    backgroundColor: BeeColors.white, borderRadius: 14, padding: 16,
    elevation: 3, shadowColor: '#000', shadowOffset: { width: 0, height: 1 },
    shadowOpacity: 0.08, shadowRadius: 4,
  },
  speciesSubtitle: {
    fontSize: 13, color: BeeColors.honey, fontWeight: '600',
    marginBottom: 12, textAlign: 'center',
  },
  speciesRow: {
    flexDirection: 'row', alignItems: 'center', paddingVertical: 8,
    borderBottomWidth: 1, borderBottomColor: BeeColors.honeycomb,
  },
  speciesRank: {
    width: 28, fontSize: 13, color: '#aaa', fontWeight: 'bold', textAlign: 'center',
  },
  speciesName: { flex: 2, fontSize: 15, color: BeeColors.honeyDark, fontWeight: '500' },
  speciesBarBg: {
    flex: 3, height: 8, backgroundColor: BeeColors.honeycomb,
    borderRadius: 4, overflow: 'hidden', marginHorizontal: 8,
  },
  speciesBarFill: { height: '100%', backgroundColor: BeeColors.honey, borderRadius: 4 },
  speciesCount: { width: 32, fontSize: 13, color: BeeColors.honey, fontWeight: 'bold', textAlign: 'right' },
  showMoreButton: {
    marginTop: 14, paddingVertical: 10, alignItems: 'center',
    borderTopWidth: 1, borderTopColor: BeeColors.honeycomb,
  },
  showMoreText: { color: BeeColors.honey, fontSize: 14, fontWeight: '600' },

  animalCard: {
    backgroundColor: BeeColors.white, borderRadius: 14, padding: 16,
    marginBottom: 24, elevation: 2, shadowColor: '#000',
    shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.08, shadowRadius: 4,
  },
  animalRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 8 },
  animalEmoji: { fontSize: 20, width: 30 },
  animalName: { flex: 2, fontSize: 14, color: BeeColors.honeyDark, fontWeight: '500' },
  animalBarBg: {
    flex: 3, height: 8, backgroundColor: BeeColors.honeycomb,
    borderRadius: 4, overflow: 'hidden', marginHorizontal: 8,
  },
  animalBarFill: { height: '100%', backgroundColor: '#f0ad4e', borderRadius: 4 },
  animalCount: { width: 28, fontSize: 14, color: '#f0ad4e', fontWeight: 'bold', textAlign: 'right' },

  emptyCard: {
    backgroundColor: BeeColors.white, borderRadius: 14, padding: 30, alignItems: 'center',
  },
